...
url = [
    "http://10.94.3.160:8080/BatchAPI",
    "http://10.94.3.161:8080/BatchAPI",
]
SMT_url = [
    "http://10.94.4.96:8080/BatchAPI",
]
MAX_RETRIES_TIMES = 3
...
class ChromeOSShopfloor(object):
    ...
    def _generic_post_with_retry(self, data, station, step, urls, bu=None):
        shuffle(urls)
        for _url in urls * MAX_RETRIES_TIMES:
            data.update({"ClientInfo": _url})
            inputstr = self.backend.ConvertToInputStr(data)
            data.update({"Inputstr": inputstr})
            if bu:
                data.update({"BU": bu})
            data.update({"Station": station, "Step": step})
    ...
    def post_with_retry(self, data, station, step):
        return self._generic_post_with_retry(data, station, step, url, bu=self.bu)

    def smt_post_with_retry(self, data, station, step):
        return self._generic_post_with_retry(data, station, step, SMT_url, bu=self.bu1)

    def sp_post_with_retry(self, data, station, step):
        return self._generic_post_with_retry(data, station, step, url, bu=self.bu2)
    ...
    def SMTStart(self, data):
        ...
        return self.smt_post_with_retry(inputstr_dict, station, step)
    ...
    def FFTStart(self, data):
        ...
        return self.sp_post_with_retry(inputstr_dict, station, step)
