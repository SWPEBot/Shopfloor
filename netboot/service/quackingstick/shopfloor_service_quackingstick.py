...
REGCODE_LOG_CSV = '/home/sysadmin/RegistrationCode/0RK/Quackingstick_registration_code_log.csv'
...
class QuantaShopfloor(object):
  ...
  def __init__(self):
    ...
    self.translate_response = {
        ...
        'in_battery': 'component.in_battery',
  ...
  def TranslateHasBattery(self, data):
    """For SMT Translate Spare Part Process."""
    key_is_in_battery = 'component.in_battery'
    value_is_in_battery = data.get(key_is_in_battery, '')
    if value_is_in_battery == 'True':
      data[key_is_in_battery] = True
    else:
      data[key_is_in_battery] = False
    return data
  ...
  def TranslateResponse(self, response):
    ...
    if 'component.in_battery' in data:
      data = self.TranslateHasBattery(data)
    return data
  ...
  def SMT_END(self, data):
    IMEI = data['factory.SIMIMEI'].strip('Quackingstick')
    LTE_EID = data['factory.LTE_EID'].strip('Quackingstick')
    extra_data = {'STATION': 'FVS',
                  'onboard_wifi': 'true',
  ...
  def FAT_START(self, data):
    extra_data = {'STATION': 'SWDLTEST',
                  ...
                  'FixtureID': data['factory.battery_sn'],
                  'MBSN': 'NOMBSN',
