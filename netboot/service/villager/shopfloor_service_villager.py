...
REGCODE_LOG_CSV = '/home/sysadmin/RegistrationCode/ZC5/Villager_registration_code_log.csv'
...
class QuantaShopfloor(object):
  ...
  def SMT_END(self, data):
    IMEI = data['factory.SIMIMEI'].strip('Villager')
    extra_data = {'STATION': 'FVS',
                  'LTE_IMEI': IMEI,
  ...
  def GRT_START(self, data):
    IMEI = data['factory.SIMIMEI'].strip('Villager')
    LTE_EID = data['factory.LTE_EID'].strip('Villager')
