...
# .........................................
# Release Date: Fri June 19 2020 19:49:47 AM
# Release By: Helin Liu
"""Implementation of ChromeOS Factory Shopfloor Service, version 2.0.13"""
...
class QuantaSharedDriveShopfloorBackend(object):
    ...
    def __init__(self, timeout_secs=10, initial_poll_secs=0.1,
        ...
        if result_key_names is None:
            ...
            result_key_names = [
                'RESULT', 'SF_CFG_CHK', 'CheckResult', ' Result']
    ...
    def CheckResponse(self, response):
        ...
        logging.info('The result keys is %s, result is %s',
                     result_keys, result)
    ...
    def Call(self, request):
        ...
        # for Retry in range(0,9):#Retry 3 times with each IP[Candy Hu-20200617]
        if request['STATION'] == 'FVS':
            ...
            for Retry in range(0, len(Host_Server_IP_FA)*MAX_RETRIES_TIMES):
                try:
                    print("try to connect %s %d time" % (
                        Host_Server_IP_FA[Retry/MAX_RETRIES_TIMES], (Retry % MAX_RETRIES_TIMES+1)))
     ...
     SELECT @ReturnValue ''' % (DataBase_SP, DataBase_Port, request['STATION'],
        ...
        try:
            ...
            logging.info(
                'DEBUG_Call: Shopfloor Response Message is "%s"', data)
        except Exception as e:
            conn.close()
            logging.info('DEBUG_Call: Operate Exception: "%s"', e)
            conn = None
            # continue
        # Release the Server-resource after data interaction[Candy Hu-20200618]
        conn.close()
...
class QuantaShopfloor(object):
    ...
    KEY_FM_CHASSIS_BRANDED = 'feature_management.chassis_branded'
    KEY_HWID = 'hwid'
    ...
    def TranslateCBX(self, data):
        """For Translate CBX Process."""
        key_is_CBX = self.KEY_FM_CHASSIS_BRANDED
        value_is_CBX = data.get(key_is_CBX, '')
        if value_is_CBX == 'True':
            data[key_is_CBX] = True
        else:
            data[key_is_CBX] = False
        return data

    # If Chromebox Need Rth MacID
    def TranslateEthernetMac(self, data):
        """Map from shopfloor MACID to DUT ethernet mac."""
        key_ethernet_mac = 'vpd.ro.ethernet_mac0'
        value_ethernet_mac = data.get(key_ethernet_mac, '').lower()
        mac = ''
        for x in range(0, 6):
            mac += value_ethernet_mac[x * 2:x * 2 + 2]
            if x != 5:
                mac += ":"
        if mac:
            data[key_ethernet_mac] = mac
        return data
    ...
    def TranslateResponse(self, response):
        ...
        if 'vpd.ro.ethernet_mac0' in data:
            data = self.TranslateEthernetMac(data)

        # for SMT translate sparepart data
        if 'component.is_spare_part' in data:
            data = self.TranslateSparePart(data)
        if 'vpd.ro.oem_device_requisition' in data:
            data[
                'vpd.ro.oem_device_requisition'] = 'remora'
        if self.KEY_FM_CHASSIS_BRANDED in data:
            data = self.TranslateCBX(data)
    ...
    def FAT_END(self, data):
        ...
        extra_data = {'STATION': 'FAT',
                      ...
                      'ERRCode': 'PASS',
                      'SN': data['serials.serial_number'],
                      'MACID': data['vpd.ro.ethernet_mac0'],
                      'MBSN': data['serials.mlb_serial_number'],
                      'MonitorAgentVer': 'VL20151102.01'}
        request = self.TranslateRequest(data, extra_data)
        return self.CallBackend(request)
    ...
    def FINALIZED(self, data):
        timeuuid = time.strftime('%Y%m%d%H%M%S',
                                 time.localtime(time.time())) + '000000'
        uuidmac = data['ethernet_mac0'] + timeuuid
        uuid = uuidmac.replace(':', "").upper()
        ...
        # Finalize Passed and send green light
        return (self.CallBackend(request), self.CallBackend(request_light))
...
def main():
    ...
    # Disable all DNS lookups, since otherwise the logging code may try to resolve IP addresses, which may delay request handling.
