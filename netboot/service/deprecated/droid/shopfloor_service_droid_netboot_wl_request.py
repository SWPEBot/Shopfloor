...
#REGCODE_LOG_CSV = '/home/sysadmin/RegistrationCode/ZBC/registration_code_log.csv'
...
class QuantaShopfloor(object):
  ...
  def __init__(self):
    ...
    self.translate_response = {
        ...
        'SKUID': 'component.sku',
        'QCI_Model': 'vpd.ro.whitelabel_tag'
    }
  ...
  def TranslateHassd(self, data):
    """For SMT Translate Spare Part Process."""
    key_is_has_sd = 'component.has_sd'
    value_is_has_sd = data.get(key_is_has_sd, '')
    if value_is_has_sd == 'True':
      data[key_is_has_sd] = True
    else:
      data[key_is_has_sd] = False
    return data
  ...
  key_is_wl = 'vpd.ro.whitelabel_tag'
  value_is_wl = data.get(key_is_wl, '')

  if value_is_wl in whitelabel_mapping:
    data[key_is_wl] = whitelabel_mapping[value_is_wl]
    return data
  else:
    del data[key_is_wl]
    return data
  ...
  def TranslateResponse(self, response):
    ...
    if 'component.has_sd' in data:
      data = self.TranslateHassd(data)
    return data
  ...
  def GRT_START(self, data):
    IMEI = data['factory.SIMIMEI'].strip('ZBC')
