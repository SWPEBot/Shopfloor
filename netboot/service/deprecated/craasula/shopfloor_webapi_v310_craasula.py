...
class HTTPShopfloorBackend(object):
    ...
    def HTTPPost(self, data, current_url):
        ...
        if "STATION=FVS" in data:
            ...
            for unused_cnt in range(0, MAX_RETRIES_TIMES):
                try:
                    headers = {
                    ...
                    if self.response:
                        ...
                        logging.info("Start next post after %s seconds", DELAY_SEC)
                        time.sleep(DELAY_SEC)

    def submit(self, data):
        _data=data
        self.response=''
        return self.MappingDeviceData(_data)
...
class ChromeOSShopfloor(object):
    ...
    def post_with_retry(self, data, station, step):
        ...
        for _url in url:
            ...
            if self.backend.HTTPPost(data, _url):
                return  self.backend.submit(self.backend.response)
