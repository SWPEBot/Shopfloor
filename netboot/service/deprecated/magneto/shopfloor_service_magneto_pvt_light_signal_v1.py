...
# ZBMA (Magneto) use the port
DEFAULT_SERVER_PORT = 11950
...
# REGCODE_LOG_CSV = '/home/sysadmin/RegistrationCode/ZAR/Kindred_registration_code_log.csv'
...
class QuantaShopfloor(object):
    ...
    def TranslateWhiteLabel(self, data):
        """For packard bell white label tag."""
        wl_k = 'vpd.ro.whitelabel_tag'
        wl_value = data.get(wl_k, 'none')
        if wl_value in [None, 'none']:
            del data[wl_k]
            return data
        if wl_value == 'Packardbell':
            data[wl_k] = 'magneto_pb'
            return data
        else:
            return data
