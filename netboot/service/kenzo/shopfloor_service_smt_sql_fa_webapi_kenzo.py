...
class HTTPShopfloorBackend(object):
    ...
    def MappingDeviceData(self, data):
        ...
        if isinstance(message_dict, dict):
            ...
            has_touchscreen = response.get("component.has_touchscreen")
            if has_touchscreen == "Y":
                response["component.has_touchscreen"] = True
            if has_touchscreen == "N":
                response["component.has_touchscreen"] = False
            logging.info("Start send device data: %s", response)
            return response
        else:
