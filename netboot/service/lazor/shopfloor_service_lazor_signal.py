...
Host_Server_IP = ['10.18.6.42', '10.18.6.41', '10.18.6.50',
                  '10.18.8.11']  # The IP group of Batch_Server

"""Path for registration code csv file Example."""
# REGCODE_LOG_CSV = '/home/sysadmin/RegistrationCode/ZC8/Lazor_registration_code_log.csv'
    ...
    def Call(self, request):
        ...
        if request['STATION'] == 'FVS':
            ...
            for Retry in range(0, 9):
                try:
                    print("try1 to connect %s %d time" %
                          (Host_Server_IP[Retry/3], (Retry % 3+1)))
...
class QuantaShopfloor(object):
    ...
    def SMT_END(self, data):
        IMEI = data['factory.SIMIMEI'].strip('Lazor')
        LTE_EID = data['factory.LTE_EID'].strip('Lazor')
        extra_data = {'STATION': 'FVS',
                      'onboard_wifi': 'true',
...
class ShopfloorService(object):
    ...
    def NotifyStart(self, data, station):
        ...
        return self._Dispatch(data, station, {
            ...
            # Yellow Light, Enter OS and D1 Passed
            'FATLightD1': self.backend.FAT_Light_D1,
