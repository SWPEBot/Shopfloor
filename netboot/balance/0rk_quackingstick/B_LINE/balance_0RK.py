# Embedded file name: balance_0rk.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-0RK.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_0RK = '0RK.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE_0RK = 'omahaserver_quackingstick.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_0RK)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_0RK)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_0RK)):
                CheckConf13()

            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_0RK)):
                CheckConf14()
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, '0RK.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_0RK)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, '0rk.log')):
        os.remove(os.path.join(BASE_PATH, '0rk.log'))
        logging.info('del logout')
    thread1 = balance('0RK', BASEFILE_0RK, INI_0RK)
    thread1.start()
