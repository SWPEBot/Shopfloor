...
def ReadIni(ini, section, option, option1, option2, option3, option4):
    try:
    ...
    def run(self):
        ...
        while not self.thread_stop:
            ...
            for count in range(len(CalSection(self.ini))):
                result = ReadIni(self.ini, 'Setting%d' % count, 'WDS11_IP', 'WDS11_PORT', 'WDS12_IP', 'WDS12_PORT', 'TIME')
                WriteConf(BASE_PATH1, self.basefile, 'http://%s:%d/update\n' % (result[0], result[1]))
                WriteConf(BASE_PATH2, self.basefile, 'http://%s:%d/update\n' % (result[2], result[3]))
                time.sleep(1)
                logging.info('**************************************************************************************')
                logging.info('%s:WDS 11 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH1, self.basefile), result[4]))
                logging.info('%s:WDS 12 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH2, self.basefile), result[4]))
                logging.info('**************************************************************************************')
                time.sleep(result[4])
