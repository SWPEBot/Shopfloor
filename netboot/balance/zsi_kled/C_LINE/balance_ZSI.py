# Embedded file name: balance_ZSI_ZSIA.py
import os
import ConfigParser
import sys
import logging
...
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASEFILE_ZSI = 'omahaserver_kled.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZSI)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZSI)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZSI)):
                CheckConf13()
...
if __name__ == '__main__':
    ...
    thread1 = balance('ZSI', BASEFILE_ZSI, INI_ZSI)
    thread1.start()
