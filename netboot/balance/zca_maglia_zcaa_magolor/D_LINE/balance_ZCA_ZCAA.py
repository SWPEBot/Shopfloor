# Embedded file name: balance_ZCA_ZCAA.py
import os
import ConfigParser
import sys
import logging
...
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASEFILE_ZCA = 'omahaserver_magolor.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZCA)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZCA)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZCA)):
                CheckConf13()
...
if __name__ == '__main__':
    ...
    thread1 = balance('ZCA', BASEFILE_ZCA, INI_ZCA)
    thread1.start()
