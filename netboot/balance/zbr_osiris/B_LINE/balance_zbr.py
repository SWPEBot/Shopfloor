# Embedded file name: balance_zbr.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-ZBR.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_ZBR = 'ZBR.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE_ZBR = 'omahaserver_osiris.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZBR)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZBR)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZBR)):
                CheckConf13()

            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZBR)):
                CheckConf14()
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, 'ZBR.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZBR)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, 'zbr.log')):
        os.remove(os.path.join(BASE_PATH, 'zbr.log'))
        logging.info('del logout')
    thread1 = balance('ZBR', BASEFILE_ZBR, INI_ZBR)
    thread1.start()
