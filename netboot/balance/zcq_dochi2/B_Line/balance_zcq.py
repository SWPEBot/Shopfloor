# Embedded file name: balance_zcq.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-zcq.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI = 'zcq2.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE = 'omahaserver_dochi2.conf'
USERNAME = 'A0010415'
    ...
    def run(self):
        ...
        while not self.thread_stop:
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, 'zcq.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, 'zcq.log')):
        os.remove(os.path.join(BASE_PATH, 'zcq.log'))
        logging.info('del logout')
    thread1 = balance('zcq', BASEFILE, INI)
    thread1.start()
