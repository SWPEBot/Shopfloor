# Embedded file name: balance_0h7.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-0H7.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_0H7 = '0H7.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE_0H7 = 'omahaserver_scout.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_0H7)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_0H7)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_0H7)):
                CheckConf13()

            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_0H7)):
                CheckConf14()
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, '0H7.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_0H7)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, '0h7.log')):
        os.remove(os.path.join(BASE_PATH, '0h7.log'))
        logging.info('del logout')
    thread1 = balance('0H7', BASEFILE_0H7, INI_0H7)
    thread1.start()
