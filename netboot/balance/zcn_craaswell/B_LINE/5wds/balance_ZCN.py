# Embedded file name: balance_zcn.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-ZCN.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_ZCN = 'ZCN.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
...
BASE_PATH4 = '/mnt/88'
BASE_PATH5 = '/mnt/89'
BASEFILE_ZCN = 'omahaserver_craaswell.conf'
USERNAME = 'A0044319'
    ...
    def run(self):
        ...
        while not self.thread_stop:
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                if os.path.exists(os.path.join(BASE_PATH, 'ZCN.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(BASE_PATH, 'ZCN.log')):
        os.remove(os.path.join(BASE_PATH, 'ZCN.log'))
        logging.info('del logout')
    thread1 = balance('ZCN', BASEFILE, INI)
    thread1.start()
