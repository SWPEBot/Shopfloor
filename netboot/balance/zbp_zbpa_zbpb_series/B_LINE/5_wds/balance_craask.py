# Embedded file name: balance_CRAASK.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-CRAASK.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI = 'craask.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
...
BASE_PATH4 = '/mnt/88'
BASE_PATH5 = '/mnt/89'
BASEFILE = 'omahaserver_craask.conf'
USERNAME = 'A0010415'
...
def ReadIni(ini, section, option, option1, option2, option3, option4, option5, option6, option7, option8, option9, option10):
    try:
        ...
        IP4 = config.get(section, option6)
        PORT4 = config.getint(section, option7)
        IP5 = config.get(section, option8)
        PORT5 = config.getint(section, option9)
        TIME = config.getint(section, option10)
    except ConfigParser.NoSectionError as e:
...
def CheckConf14():
    global BASE_PATH4
    commands.getoutput('umount %s' % BASE_PATH4)
    RunRsync('again:mount -t cifs -o username=%s,domain=%s,password=%s //%s/reminst %s' % (USERNAME,
     DOMAIN,
     PASSWORD,
     WDSIP4,
...
def CheckConf15():
    global BASE_PATH5
    commands.getoutput('umount %s' % BASE_PATH5)
    RunRsync('again:mount -t cifs -o username=%s,domain=%s,password=%s //%s/reminst %s' % (USERNAME,
     DOMAIN,
     PASSWORD,
     WDSIP5,
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            ...
            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE)):
                CheckConf14()

            while not os.path.exists(os.path.join(BASE_PATH5, BASEFILE)):
                CheckConf15()

            for count in range(len(CalSection(self.ini))):
                result = ReadIni(self.ini, 'Setting%d' % count, 'WDS11_IP', 'WDS11_PORT', 'WDS12_IP', 'WDS12_PORT', 'WDS13_IP', 'WDS13_PORT', 'WDS14_IP', 'WDS14_PORT', 'WDS15_IP', 'WDS15_PORT', 'TIME')
                ...
                WriteConf(BASE_PATH4, self.basefile, 'http://%s:%d/update\n' % (result[6], result[7]))
                WriteConf(BASE_PATH5, self.basefile, 'http://%s:%d/update\n' % (result[8], result[9]))
                time.sleep(1)
                logging.info('**************************************************************************************')
                logging.info('%s:WDS 11 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH1, self.basefile), result[10]))
                logging.info('%s:WDS 12 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH2, self.basefile), result[10]))
                logging.info('%s:WDS 13 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH3, self.basefile), result[10]))
                logging.info('%s:WDS 14 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH4, self.basefile), result[10]))
                logging.info('%s:WDS 15 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH5, self.basefile), result[10]))
                logging.info('**************************************************************************************')
                time.sleep(result[10])
                if os.path.exists(os.path.join(BASE_PATH, 'CRAASK.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(BASE_PATH, 'CRAASK.log')):
        os.remove(os.path.join(BASE_PATH, 'CRAASK.log'))
        logging.info('del logout')
    thread1 = balance('CRAASK', BASEFILE, INI)
    thread1.start()
