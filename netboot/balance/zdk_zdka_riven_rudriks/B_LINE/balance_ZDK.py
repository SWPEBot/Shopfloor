# Embedded file name: balance_zdk.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-ZDK.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_ZDK = 'ZDK.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE_ZDK = 'omahaserver_riven.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZDK)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZDK)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZDK)):
                CheckConf13()

            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZDK)):
                CheckConf14()
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, 'ZDK.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZDK)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, 'zdk.log')):
        os.remove(os.path.join(BASE_PATH, 'zdk.log'))
        logging.info('del logout')
    thread1 = balance('ZDK', BASEFILE_ZDK, INI_ZDK)
    thread1.start()
