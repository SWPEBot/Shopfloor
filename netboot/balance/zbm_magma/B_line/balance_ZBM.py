# Embedded file name: balance_zbm.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-ZBM.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_ZBM = 'ZBM.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE_ZBM = 'omahaserver_magma.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZBM)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZBM)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZBM)):
                CheckConf13()

            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZBM)):
                CheckConf14()
            ...
            for count in range(len(CalSection(self.ini))):
                ...
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, 'ZBM.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZBM)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, 'zbm.log')):
        os.remove(os.path.join(BASE_PATH, 'zbm.log'))
        logging.info('del logout')
    thread1 = balance('ZBM', BASEFILE_ZBM, INI_ZBM)
    thread1.start()
