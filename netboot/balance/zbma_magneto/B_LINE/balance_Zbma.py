# Embedded file name: balance_zbma.py
import os
import ConfigParser
import sys
import logging
...
import commands
import subprocess
logging.basicConfig(filename='/tmp/test-ZBMA.log', format='%(asctime)s:%(levelname)s:%(message)s', level=logging.DEBUG)
INI_ZBMA = 'ZBMA.ini'
BASE_PATH = '/mnt/star'
BASE_PATH1 = '/mnt/55'
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASE_PATH4 = '/mnt/88'
BASEFILE_ZBMA = 'omahaserver_magneto.conf'
USERNAME = 'A0044319'
...
def ReadIni(ini, section, option, option1, option2, option3, option4, option5, option6, option7, option8):
    try:
        ...
        TIME = config.getint(section, option8)
    except ConfigParser.NoSectionError as e:
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZBMA)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZBMA)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZBMA)):
                CheckConf13()

            while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZBMA)):
                CheckConf14()

            for count in range(len(CalSection(self.ini))):
                result = ReadIni(self.ini, 'Setting%d' % count, 'WDS11_IP', 'WDS11_PORT', 'WDS12_IP', 'WDS12_PORT', 'WDS13_IP', 'WDS13_PORT', 'WDS14_IP', 'WDS14_PORT', 'TIME')
                ...
                time.sleep(1)
                logging.info('**************************************************************************************')
                logging.info('%s:WDS 11 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH1, self.basefile), result[8]))
                logging.info('%s:WDS 12 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH2, self.basefile), result[8]))
                logging.info('%s:WDS 13 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH3, self.basefile), result[8]))
                logging.info('%s:WDS 14 server write %s in conf file,delay %ds' % (self.model, ReadConf(BASE_PATH4, self.basefile), result[8]))
                logging.info('**************************************************************************************')
                time.sleep(result[8])
                if os.path.exists(os.path.join(BASE_PATH, 'ZBMA.log')):
                    logging.info('logout')
                    self.thread_stop = True
                    break
...
if __name__ == '__main__':
    ...
    while not os.path.exists(os.path.join(BASE_PATH4, BASEFILE_ZBMA)):
        ...
        time.sleep(5)
    if os.path.exists(os.path.join(BASE_PATH, 'zbma.log')):
        os.remove(os.path.join(BASE_PATH, 'zbma.log'))
        logging.info('del logout')
    thread1 = balance('ZBMA', BASEFILE_ZBMA, INI_ZBMA)
    thread1.start()
