# Embedded file name: balance_ZAC_ZACA.py
import os
import ConfigParser
import sys
import logging
...
BASE_PATH2 = '/mnt/66'
BASE_PATH3 = '/mnt/77'
BASEFILE_ZAC = 'omahaserver_magister.conf'
USERNAME = 'A0044319'
...
class balance(threading.Thread):
    ...
    def run(self):
        ...
        while not self.thread_stop:
            while not os.path.exists(os.path.join(BASE_PATH1, BASEFILE_ZAC)):
                CheckConf11()

            while not os.path.exists(os.path.join(BASE_PATH2, BASEFILE_ZAC)):
                CheckConf12()

            while not os.path.exists(os.path.join(BASE_PATH3, BASEFILE_ZAC)):
                CheckConf13()
...
if __name__ == '__main__':
    ...
    thread1 = balance('ZAC', BASEFILE_ZAC, INI_ZAC)
    thread1.start()
