...
"""Implementation of ChromeOS Factory Shopfloor Service, version 3.1.6"""
...
# ZCT (baze)   use the port
MODEL_TRACE_NAME="ZCT_Trace"
LOG_SUFFIX=".csv"
MODEL_TRACE_DIR="/home/sysadmin/MODEL_TRACE/"
WL_SN_LOG_PATH=MODEL_TRACE_DIR+MODEL_TRACE_NAME+LOG_SUFFIX
BATT_STORAGE_DIR="/home/sysadmin/BATT_STORAGE/"
BATT_STORAGE_LOG_PATH=BATT_STORAGE_DIR+"ZCT"+LOG_SUFFIX
ERROR_MSG_LOG_PATH="/home/sysadmin/error_msg.txt"
...
def write_error_msg(station, ip,sn, message):
    try:
        with open(ERROR_MSG_LOG_PATH, 'a', encoding="utf-8") as f:
            f.write(time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime())+" ZCT: "+sn+": "+station+": "+ip+": "+ "get response: "+ message+"\n")
    except:
        pass

def save_batt_storage(sn,ShipC,storage_type,typeA_est,typeB_est,pre_eol,ssd_t,ssd_c,ssd_s):
    header = ['Time', 'SN', 'Battery Count', 'Storage Type', 'TypeA EST', 'TypeB EST', 'Pre EOL', 'SSD Power Hours', 'SSD Power Count', 'SSD Unsafe Shutdowns']
    file_exists = os.path.isfile(BATT_STORAGE_LOG_PATH)
    os.makedirs(BATT_STORAGE_DIR, exist_ok=True)
    try:
        with file_lock:
            with open(BATT_STORAGE_LOG_PATH, 'a+') as f:
                writer = csv.writer(f,dialect=NewlineTerminatedCSVDialect)
                if not file_exists:
                    writer.writerow(header)
                writer.writerow(
                    [time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime()),sn,ShipC,storage_type, typeA_est,typeB_est,pre_eol,ssd_t, ssd_c, ssd_s])
...
class HTTPShopfloorBackend(object):
    ...
    def MappingDeviceData(self, data):
        ...
        device_data_mapping = {
            ...
            "LINE": "factory.line",
            'chassis_branded': "feature_management.chassis_branded",
           # "LTE_EID": "factory.LTE_EID",
           # "LTE_HW": "factory.CLLHW",
           # "LTE_FW": "factory.CLLFW",
            "SKUID": "component.sku",
            "dlm_sku_id": "vpd.ro.dlm_sku_id",
        }
        ...
        if isinstance(message_dict, dict):
            ...
            cbx = response.get("feature_management.chassis_branded")
            if is_spare_part == "Y":
                response["component.is_spare_part"] = True
            if is_spare_part == "N":
                response["component.is_spare_part"] = False
            if cbx == "False":
                response["feature_management.chassis_branded"] = False
            if cbx == "True":
                response["feature_management.chassis_branded"] = True
    ...
    def HTTPPost(self, data, current_url):
        ...
        if "STATION=FVS" in data:
            ...
            try:
                ...
                # Collect error message for all stations
                if "Fail" in message or "ErrorMsg" in message:
                    if request_sn:
                        write_error_msg(station, current_url,request_sn, message)
                    else:
                        write_error_msg(station, current_url,mac_id, message)

                # Collect PASS reponse for SWDL
                if station == "SWDL":
...
class ChromeOSShopfloor(object):
    ...
    def sppost_with_retry(self, data):
        """try next web api url when current post got an error"""
        _urls=url
        shuffle(_urls)
        for _url in _urls*MAX_RETRIES_TIMES:
    ...
    def FFTStart(self, data):
        inputstr_dict = {
            ...
            "WLANID": data["factory.wifi_mac"],
            "BT_MAC": data["factory.bluetooth_mac"],
            "mlb_serial_number": data["serials.mlb_serial_number"],
            "Step": "Handshake",
            "ErrCode": "PASS",
            "MonitorAgentVer": "WEBAPI2.0.0.1",
        }
        return self.sppost_with_retry(inputstr_dict)
    ...
    def GRTStart(self, data):
        ...
        ssd_c='N/A'
        ssd_s ='N/A'
        shipC=data['factory.ShipBat']
      # shipC='N/A'
        if 'SSD' in data['factory.HD1H']:
            storage_type='SSD'
            ssd_c, ssd_t, ssd_s=data['factory.HD1H'].strip('SSD ').split(' ')
        elif 'UFS' in data['factory.HD1H']:
            storage_type='UFS'
            typeA_est, typeB_est, pre_eol=[ x.split('=')[1] for x in data['factory.HD1H'].strip('UFS ').split(' ')]
        elif 'EMMC' in data['factory.HD1H']:
            storage_type='EMMC'
            typeA_est, typeB_est, pre_eol=[ x.split('=')[1] for x in data['factory.HD1H'].strip('EMMC ').split(' ')]
        else:
            pass
        
        save_batt_storage(data["serials.serial_number"],shipC,storage_type,typeA_est,typeB_est,pre_eol,ssd_t,ssd_c,ssd_s)
        ...
        inputstr = {
            ...
            "BatC": data["factory.BatC"],
            "ShipBat": data["factory.ShipBat"],
            "HD1H": data["factory.HD1H"],
            "MBSN": data["serials.mlb_serial_number"],
